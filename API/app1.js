...
app.post('/persona', async ( req , res ) => 
{
    try{
        ...
        let imail = req.body.imail ; 
        let alias = req.body.alias ;
        if(nombre.length==0 ||apellido.length==0 ||imail.length==0 || alias.length==0 )
        {
            throw new Error ('Se envio información vacia')
        }
...
app.put('/persona/:id', async ( req , res ) => 
{
    try
    {
       ...
       /* if(nombre.length==0 ||apellido.length==0 ||alias.length==0  )
        {
            throw new Error ('Se envio información vacia')
        }
        ...
        query = 'UPDATE persona SET nombre = ?, apellido = ?, alias = ?, imail = ? WHERE id = ?' ; 
        respuesta = await qy ( query, [nombre, apellido, alias, imail, id] ) ; 
        res.status(200).send({'respuesta':respuesta}) ;
    }
    catch(e)
    {
